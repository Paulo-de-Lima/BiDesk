...
public class ManutencaoView extends JPanel {
    ...
    private void initializeComponents() {
        ...
        tabelaPendentes.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                int row = tabelaPendentes.rowAtPoint(e.getPoint());
                int col = tabelaPendentes.columnAtPoint(e.getPoint());
                
                if (row >= 0 && row < tabelaPendentes.getRowCount() && col >= 0 && col < tabelaPendentes.getColumnCount()) {
                    // Exibir conteúdo completo ao clicar em qualquer célula (exceto ações)
                    if (col != 3) { // Não mostrar para coluna de ações
                        Object value = tabelaPendentes.getValueAt(row, col);
                        if (value != null) {
                            String texto = value.toString();
                            String nomeColuna = tabelaPendentes.getColumnName(col);
